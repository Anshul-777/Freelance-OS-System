          ...
          {projects.length === 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <div className="bg-gradient-to-br from-blue-50 to-indigo-50 p-6 rounded-xl border border-blue-200">
                <div className="w-10 h-10 bg-blue-200 rounded-lg flex items-center justify-center mb-4">
                  <FolderOpen size={20} className="text-blue-600" />
                </div>
                <h4 className="font-bold text-slate-900 mb-2">Project Overview</h4>
                <p className="text-sm text-slate-700 leading-relaxed">
                  Track client name, budget, due dates, and billable status. Each project is a real workspace for your work.
                </p>
              </div>
              
              <div className="bg-gradient-to-br from-emerald-50 to-teal-50 p-6 rounded-xl border border-emerald-200">
                <div className="w-10 h-10 bg-emerald-200 rounded-lg flex items-center justify-center mb-4">
                  <Clock size={20} className="text-emerald-600" />
                </div>
                <h4 className="font-bold text-slate-900 mb-2">Time & Money Tracking</h4>
                <p className="text-sm text-slate-700 leading-relaxed">
                  Log time entries and expenses directly to projects. See total hours, costs, and profit estimates automatically.
                </p>
              </div>
              
              <div className="bg-gradient-to-br from-amber-50 to-orange-50 p-6 rounded-xl border border-amber-200">
                <div className="w-10 h-10 bg-amber-200 rounded-lg flex items-center justify-center mb-4">
                  <DollarSign size={20} className="text-amber-600" />
                </div>
                <h4 className="font-bold text-slate-900 mb-2">Deliverables & Scope</h4>
                <p className="text-sm text-slate-700 leading-relaxed">
                  Define deliverables, track revisions, and log scope changes with their budget/timeline impact. Protect yourself from scope creep.
                </p>
              </div>
              
              <div className="bg-gradient-to-br from-purple-50 to-pink-50 p-6 rounded-xl border border-purple-200">
                <div className="w-10 h-10 bg-purple-200 rounded-lg flex items-center justify-center mb-4">
                  <Calendar size={20} className="text-purple-600" />
                </div>
                <h4 className="font-bold text-slate-900 mb-2">Deadline Tracking</h4>
                <p className="text-sm text-slate-700 leading-relaxed">
                  Get risk warnings when projects are overdue or falling behind. See countdown timers and completion estimates.
                </p>
              </div>
              
              <div className="bg-gradient-to-br from-red-50 to-rose-50 p-6 rounded-xl border border-red-200">
                <div className="w-10 h-10 bg-red-200 rounded-lg flex items-center justify-center mb-4">
                  <Clock size={20} className="text-red-600" />
                </div>
                <h4 className="font-bold text-slate-900 mb-2">Task Management</h4>
                <p className="text-sm text-slate-700 leading-relaxed">
                  Use Kanban boards to organize tasks, track progress, and see what's in progress, review, or done.
                </p>
              </div>
              
              <div className="bg-gradient-to-br from-cyan-50 to-blue-50 p-6 rounded-xl border border-cyan-200">
                <div className="w-10 h-10 bg-cyan-200 rounded-lg flex items-center justify-center mb-4">
                  <FolderOpen size={20} className="text-cyan-600" />
                </div>
                <h4 className="font-bold text-slate-900 mb-2">File Management</h4>
                <p className="text-sm text-slate-700 leading-relaxed">
                  Upload and organize contracts, briefs, deliverables, and references. Keep everything in one place.
                </p>
              </div>
            </div>
          )}
                ...
                <tr key={p.id} className="table-row">
                  <td className="table-td">
                    <div className="flex items-center gap-3">
                      <div className="w-1.5 h-10 rounded-full dynamic-bg" style={{ '--dynamic-bg-color': p.color } as React.CSSProperties} />
                      <div>
                  ...
                  <td className="table-td w-32">
                    <ProgressBar value={Number(p.completion_percentage) || 0} max={100} />
    ...
    >
      ...
      <div className="mb-4">
        <div className="flex justify-between text-xs text-slate-500 mb-1.5">
          <span>Progress</span>
          <span className="font-semibold">{(Number(p.completion_percentage) || 0).toFixed(0)}%</span>
        </div>
        <ProgressBar
          value={Number(p.completion_percentage) || 0}
          max={100}
          color={Number(p.completion_percentage) >= 100 ? 'bg-emerald-500' : 'bg-brand-600'}
        />
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 pt-3 border-t border-slate-100">
        <div>
          ...
          <p className="text-sm font-semibold text-slate-900">{(Number(p.total_hours) || 0).toFixed(1)}h</p>
        </div>
        <div>
          <p className="text-xs text-slate-400">Earned</p>
          <p className="text-sm font-semibold text-slate-900">
            {Number(p.total_earnings) > 0 ? `$${(Number(p.total_earnings) / 1000).toFixed(1)}k` : '—'}
          </p>
        </div>
        <div>
