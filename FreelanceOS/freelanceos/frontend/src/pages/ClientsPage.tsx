    ...
    <div className="page-container max-w-[1600px]">
      ...
      <div className="flex gap-6 h-[calc(100vh-12rem)]">
        ...
        <div className="w-72 flex-shrink-0 flex flex-col gap-3">
          <div className="relative">
            ...
            <input
              ...
              className="input-field pl-10 py-2.5 font-medium"
            />
          </div>
            ...
            {filtered.length === 0 ? (
              <div className="card">
                <EmptyState
                  ...
                  action={<button onClick={openCreate} className="btn-primary text-sm mt-3">+ Add Client</button>}
                />
              </div>
            ) : (
              filtered.map((c) => (
                <button
                  ...
                  className={classNames(
                    'w-full text-left card p-4 hover:shadow-lg transition-all duration-200 border-2 hover:border-slate-200',
                    selected?.id === c.id ? 'ring-2 ring-brand-500 shadow-lg bg-brand-50 border-brand-300' : 'border-slate-100 hover:border-brand-200'
                  )}>
                  <div className="flex items-center gap-3">
                    <div className="avatar w-11 h-11 text-sm flex-shrink-0 font-bold text-lg"
                    ...
                    <div className="text-right flex-shrink-0">
                      ...
                      <p className="text-xs text-slate-400 font-semibold">{c.total_projects}p</p>
                    </div>
                  </div>
                </button>
              ))
        ...
        <div className="flex-1 overflow-y-auto">
          {!selected ? (
            <div className="card h-full flex flex-col items-center justify-center bg-gradient-to-b from-slate-50 to-white">
              <Users size={48} className="text-slate-300 mb-4" />
              <h2 className="text-2xl font-bold text-slate-600 mb-2">Select a client</h2>
              <p className="text-slate-400 mb-6 text-center max-w-sm">Click on a client in the list to view their complete profile, projects, communication history, and financial details.</p>
              <button onClick={openCreate} className="btn-primary">+ Add Your First Client</button>
            </div>
          ) : (
              ...
              <div className="card p-6 hover:shadow-lg transition-all">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-4">
                    <div className="avatar w-16 h-16 text-xl dynamic-bg"
                      style={{ '--dynamic-bg-color': getAvatarColor(selected.name) } as React.CSSProperties}>
