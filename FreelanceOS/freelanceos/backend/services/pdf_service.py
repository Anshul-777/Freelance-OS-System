...
def generate_invoice_pdf(invoice, user) -> bytes:
    ...
    paid     = Decimal(str(invoice.amount_paid or "0.0"))
    tax_rate = invoice.tax_rate or 0
    tax_amt  = Decimal(str(invoice.tax_amount or "0.0"))
    disc     = Decimal(str(invoice.discount_amount or "0.0"))
    total    = Decimal(str(invoice.total or "0.0"))
    subtot   = Decimal(str(invoice.subtotal or "0.0"))
    ...
    totals_rows = [
        [Paragraph("Subtotal", totals_style), Paragraph(_currency(subtot), totals_val_style)],
    ]
    if tax_rate:
        totals_rows.append([
